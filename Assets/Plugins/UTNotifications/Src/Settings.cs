...
namespace UTNotifications
{
    ...
    public class Settings : ScriptableObject
    {
        ...
        {
#if UNITY_EDITOR_WIN
            var homeDir = System.Environment.ExpandEnvironmentVariables("%HOMEDRIVE%%HOMEPATH%");
            var javaHome = JavaHome;

            if (string.IsNullOrEmpty(javaHome))
            {
                var error = "<JDK path not found. Please make sure that JDK is installed>";
                Debug.LogError(error);
                return new string[] { error, error };
            }

            var keytool = javaHome + "\\bin\\keytool.exe";
#else
            string homeDir = System.Environment.GetEnvironmentVariable("HOME");
            string keytool = "keytool";
#endif

            var debugKeystore = Path.Combine(homeDir, ".android/debug.keystore");
            if (!File.Exists(debugKeystore))
            {
                var error = "<debug.keystore file not found. Please build an Android version at least once.>";
                Debug.LogError(error);
                return new string[] { error, error };
            }

            var keytoolOutput = RunCommand(keytool, "-list -v -alias androiddebugkey -storepass android -keystore \"" + debugKeystore + "\"");
            var indexMD5 = keytoolOutput.IndexOf("MD5:");
            var indexSHA256 = keytoolOutput.IndexOf("SHA256:");

            if (indexMD5 < 0 || indexSHA256 < 0)
            {
                var message = "<Unable to read \"debug.keystore\" file. Look http://stackoverflow.com/questions/8576732/there-is-no-debug-keystore-in-android-folder >";
        ...
        {
            var file = Path.Combine(PluginsFullPath, "Android/assets/api_key.txt");
        ...
        public static void SetAmazonAPIKey(string value)
        {
            ...
            {
                var dir = Path.Combine(PluginsFullPath, "Android/assets");
...
#pragma warning restore 414
        ...
        private static void Update()
        {
            ...
            var resourcesFullPath = FullPath(AssetsRelatedResourcesPath);
        ...
        {
            var targetFileName = path + "/." + extenstion;
            if (!File.Exists(targetFileName))
            {
                var originalFileName = path + "/eclipse_adt." + extenstion;
        ...
        {
            var manifestFile = Settings.MainAndroidManifestFullPath;
        ...
        {
			var updateAssetDatabase = false;
			
            var facebookAndroidLibsPath = Path.Combine(Application.dataPath, "FacebookSDK/Plugins/Android/libs");

            if (Directory.Exists(facebookAndroidLibsPath))
            {
                var facebookSupportLibraryFiles = Directory.GetFiles(facebookAndroidLibsPath, "support-v4-*");

                if (facebookSupportLibraryFiles != null)
                {
                    foreach (var it in facebookSupportLibraryFiles)
                    {
        ...
        {
            var resManifestFile = Path.Combine(GetAndroidResourceLibFolder(), "AndroidManifest.xml");
            if (File.Exists(resManifestFile) && !File.ReadAllText(resManifestFile).Contains("android:minSdkVersion"))
            {
                var defaultResManifestFile = Path.Combine(Settings.FullPath(Settings.AssetsRelatedEditorPath), "Android/Res/AndroidManifest.xml");
        ...
        private void CheckProguard()
        {
            ...
            {
                var proguardFileName = Path.Combine(PluginsFullPath, "Android/proguard-user.txt");
                if (!File.Exists(proguardFileName))
                {
                    Debug.Log("UTNotifications: Enabling User Proguard File");

                    var proguardFileNameDisabled = proguardFileName + ".DISABLED";
                ...
                var updatedProguardFileText = AddLineIfMissing(proguardFileText, "# UTNotifications Proguard Config (please do not edit)");
        ...
        private void CheckWSAPlugin()
        {
            ...
            var metroPath = Path.Combine(PluginsFullPath, "Metro");
            var wsaPath = Path.Combine(PluginsFullPath, "WSA");
            string rightPath;
            string wrongPath;
#if !UNITY_4
            rightPath = wsaPath;
            ...
            if (Directory.Exists(wrongPath))
            {
                ...
                var filesToMove = Directory.GetFiles(wrongPath, "UTNotifications.*");
        ...
        private void CheckWSADllPlatforms()
        {
            ...
            var pluginPath = "Assets/Plugins/UTNotifications.dll";
        ...
        private void CheckAssetVersionUpdated()
        {
            ...
            {
                var updateMessage = "";
                var savedVersionIndex = -1;
                for (var i = 0; i < m_assetBundleUpdateMessages.Length; ++i)
                {
                ...
                for (var i = savedVersionIndex + 1; i < m_assetBundleUpdateMessages.Length; ++i)
                {
        ...
        private bool MoveFilesStartingWith(string pathFrom, string pathTo, string startsWith)
        {
            ...
            if (Directory.Exists(pathFrom))
            {
                ...
                var files = Directory.GetFiles(pathFrom, startsWith + "*");
                if (files != null)
                {
                    foreach (var file in files)
                    {
                        ...
                        var fullPathTo = pathTo + "/" + Path.GetFileName(file);
        ...
        {
            using (var subkey = key.OpenSubKey(@"SOFTWARE\JavaSoft\Java Development Kit"))
            {
                if (subkey == null)
                {
                    return null;
                ...
                var value = subkey.GetValue("CurrentVersion", null, Microsoft.Win32.RegistryValueOptions.None);
                if (value != null)
                {
                    using (var currentHomeKey = subkey.OpenSubKey(value.ToString()))
                    {
