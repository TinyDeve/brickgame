...
namespace UTNotifications
{
    ...
    //public
        public static void Update()
        {
            ...
            {
                //prior Unity 5.2
                var defaultManifest = Path.Combine(EditorApplication.applicationContentsPath, "PlaybackEngines/androidplayer/AndroidManifest.xml");
            ...
            var manifestNode = XmlUtils.FindChildNode(xmlDocument, "manifest") as XmlElement;
            var applicationNode = XmlUtils.FindChildNode(manifestNode, "application");
            ...
            var ns = applicationNode.GetNamespaceOfPrefix("android");
    ...
    //private
        ...
        {
            var comment = "Android 4.4 Compatibility Mode";
        ...
        {
            var comment = "Firebase Cloud Messaging";
            ...
            if (Settings.Instance.PushNotificationsEnabledFirebase)
            {
                ...
                {
                    var receiverElement = XmlUtils.UpdateOrCreateElement(xmlDocument, applicationNode, "receiver", "name", ns, "com.google.android.gms.gcm.GcmReceiver", comment);
                    ...
                    //intent-filter
                    var intentFilter = XmlUtils.UpdateOrCreateElement(xmlDocument, receiverElement, "intent-filter");
                ...
                {
                    var serviceElement = XmlUtils.UpdateOrCreateElement(xmlDocument, applicationNode, "service", "name", ns, "universal.tools.notifications.GcmIntentService");
                    serviceElement.SetAttribute("exported", ns, "false");

                    //intent-filter
                    var intentFilter = XmlUtils.UpdateOrCreateElement(xmlDocument, serviceElement, "intent-filter");
                ...
                {
                    var serviceElement = XmlUtils.UpdateOrCreateElement(xmlDocument, applicationNode, "service", "name", ns, "universal.tools.notifications.GcmInstanceIDListenerService");
                    serviceElement.SetAttribute("exported", ns, "false");
            ...
            if (Settings.Instance.PushNotificationsEnabledFirebase)
            {
                var permission = XmlUtils.UpdateOrCreateElement(xmlDocument, manifestNode, "permission", "name", ns, ApplicationIdentifier() + ".permission.C2D_MESSAGE", comment);
        ...
        {
            var comment = "Amazon Device Messaging";

            //<manifest xmlns:amazon="http://schemas.amazon.com/apk/res/android" />
            var xmlns = manifestNode.GetNamespaceOfPrefix("xmlns");
            var amazonNs = "http://schemas.amazon.com/apk/res/android";
            ...
            if (Settings.Instance.PushNotificationsEnabledAmazon)
            {
                ...
                var enableFeature = XmlUtils.UpdateOrCreateElement(xmlDocument, applicationNode, "amazon:enable-feature", "name", ns, "com.amazon.device.messaging", comment, amazonNs);
                enableFeature.SetAttribute("required", ns, "false");

                //receiver
                var receiverElement = XmlUtils.UpdateOrCreateElement(xmlDocument, applicationNode, "receiver", "name", ns, "universal.tools.notifications.AdmBroadcastReceiver");
                ...
                //service
                var service = XmlUtils.UpdateOrCreateElement(xmlDocument, applicationNode, "service", "name", ns, "universal.tools.notifications.AdmIntentService");
            ...
            if (Settings.Instance.PushNotificationsEnabledAmazon)
            {
                var permission = XmlUtils.UpdateOrCreateElement(xmlDocument, manifestNode, "permission", "name", ns, ApplicationIdentifier() + ".permission.RECEIVE_ADM_MESSAGE", comment);
                permission.SetAttribute("protectionLevel", ns, "signature");
        ...
        {
            var comment = "Restore Scheduled Notifications On Reboot";
            ...
            {
                //receiver and comment
                var receiverElement = XmlUtils.UpdateOrCreateElement(xmlDocument, applicationNode, "receiver", "name", ns, "universal.tools.notifications.ScheduledNotificationsRestorer", comment);
