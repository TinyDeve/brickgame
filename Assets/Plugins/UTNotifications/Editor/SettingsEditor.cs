...
namespace UTNotifications
{
    ...
    //private
        ...
        private void DrawNotificationProfilesGUI()
        {
            ...
            {
                var profiles = Settings.Instance.NotificationProfiles;
        ...
        private void DrawFirebaseSettingsGUI()
        {
            ...
            {
                var firebaseWasEnabled = Settings.Instance.PushNotificationsEnabledFirebase;
                var firebaseNowEnabled = EditorGUILayout.Toggle(m_firebasePushNotificationsEnabledLabel, firebaseWasEnabled);
        ...
        private void DrawAmazonSettingsGUI()
        {
            if (m_showAmazonSettings = EditorGUILayout.Foldout(m_showAmazonSettings, "Amazon Device Messaging"))
            {
                if (Settings.Instance.PushNotificationsEnabledAmazon = EditorGUILayout.Toggle(m_amazonPushNotificationsEnabledLabel, Settings.Instance.PushNotificationsEnabledAmazon))
                {
                    ...
                    var apiKey = EditorGUILayout.TextArea(m_amazonDebugAPIKey, GUILayout.Width(width));
        ...
        private void DrawWindowsSettingsGUI()
        {
            if (m_showWindowsSettings = EditorGUILayout.Foldout(m_showWindowsSettings, "Windows Store"))
            {
                ...
                {
                    var certificatePublisher = Settings.Instance.WindowsCertificatePublisher;
                    EditorGUILayout.BeginHorizontal();
        ...
        private void LoadGoogleServicesJson()
        {
            ...
            var filePath = EditorUtility.OpenFilePanel("Open google-services.json", "", "json");
            if (!string.IsNullOrEmpty(filePath))
            {
                var json = JSON.Parse(File.ReadAllText(filePath));
                ...
                try
                {
                    ...
                    var found = false;
                    for (var i = 0; i < clientNode.Count; ++i)
                    {
    ...
    //public
        ...
        {
            var dialog = (NotificationProfileDialog)EditorWindow.GetWindow(typeof(NotificationProfileDialog));
    ...
    //private
        ...
        private void DrawAndroid()
        {
            if (m_showAndroid = EditorGUILayout.Foldout(m_showAndroid, "Android"))
            {
                ...
                var name = string.IsNullOrEmpty(m_profile.androidChannelName) ? m_profile.profileName : m_profile.androidChannelName;
                ...
                var colorSpecified = EditorGUILayout.Toggle(m_android5PlusColorLabel, m_profile.colorSpecified);
        ...
        private bool CheckProfileName()
        {
            ...
            {
                foreach (var profile in Settings.Instance.NotificationProfiles)
                {
        ...
        private void DrawFileOption(GUIContent label, ref string fileName, string[] extenstions, string helpUrl)
        {
            ...
            if (GUILayout.Button(string.IsNullOrEmpty(fileName) ? "Default (choose: *." + extensionsAsString + ")" : new FileInfo(fileName).Name, GUILayout.Width(215)))
            {
                while (true)
                {
                    var selectedFileName = EditorUtility.OpenFilePanel(label.text.Replace(" [?]:", "") + " (" + extensionsAsString + ")", "", extensionString);
        ...
        private void Apply()
        {
            ...
            var profileName = m_profile.profileName != Settings.DEFAULT_PROFILE_NAME ? m_profile.profileName : Settings.DEFAULT_PROFILE_NAME_INTERNAL;
        ...
        private void RenameIOSResource(string resourceType, string oldName, string newName)
        {
            ...
            var iosResPath = Settings.GetIOSResourceFolder();
        ...
        private void RenameAndroidResource(string resourceType, string oldName, string newName)
        {
            ...
            var androidResPath = Settings.GetAndroidResourceFolder(resourceType);
            if (Directory.Exists(androidResPath))
            {
                var fileInfos = new DirectoryInfo(androidResPath).GetFiles(oldName + ".*");
                if (fileInfos != null && fileInfos.Length > 0)
                {
                    var newPath = Path.Combine(androidResPath, newName + fileInfos[0].Extension);
        ...
        private static void PrepareAndroidResource()
        {
            ...
            var androidResLibPath = Settings.GetAndroidResourceLibFolder();
            if (!Directory.Exists(androidResLibPath))
            {
                Directory.CreateDirectory(androidResLibPath);
            }

            var resLibTemplatePath = Path.Combine(Settings.FullPath(Settings.AssetsRelatedEditorPath), "Android/Res");
            var fileInfos = new DirectoryInfo(resLibTemplatePath).GetFiles();

            foreach (var fileInfo in fileInfos)
            {
                if (!fileInfo.Name.StartsWith(".") && !fileInfo.Name.EndsWith(".meta"))
                {
                    var targetFileName = Path.Combine(androidResLibPath, fileInfo.Name);
        ...
        private static void CopyAndroidResource(string resourceType, string originalPath, string outName)
        {
            ...
            if (Directory.Exists(androidResPath))
            {
                var fileInfos = new DirectoryInfo(androidResPath).GetFiles(outName + ".*");
                if (fileInfos != null)
                {
                    foreach (var fileInfo in fileInfos)
                    {
